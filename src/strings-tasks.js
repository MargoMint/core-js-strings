...
function concatenateStrings(value1, value2) {
  const resultConcatenateStrings = '';
  return resultConcatenateStrings.concat(value1, value2);
}
...
function removeFirstOccurrences(str, value) {
  ...
  return (
    str.substring(0, indexOfFirst) + str.substring(indexOfFirst + value.length)
  );
}
...
function formatTime(minutes, seconds) {
  const nexMinutes = String(minutes).padStart(2, '0');
  const newSeconds = String(seconds).padStart(2, '0');
  return `${nexMinutes}:${newSeconds}`;
}
...
function countVowels(str) {
  ...
  const sum = str.split('').filter((i) => vowels.includes(i));
  return sum.length;
}
...
function isPalindrome(str) {
  ...
  return cleanedStr === PalindromeStr;
}
...
function encodeToRot13(str) {
  ...
  const newStr = str.split('').map((i) => {
    if (allLower.includes(i)) {
      const indexOfI = allLower.indexOf(i);
      return allLower[(indexOfI + 13) % 26];
    }
    if (allUpper.includes(i)) {
      const indexOfI = allUpper.indexOf(i);
      return allUpper[(indexOfI + 13) % 26];
    }
    return i;
  });
  return newStr.join('');
